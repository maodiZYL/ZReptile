...
type Wutuobang8 struct {
	Members []struct {
		ID string `json:"id"`
	} `json:"members"`
}

type tongxing8 struct {
...
func Tongxing9(bang string, bang2 string) { //bang为结构体，接收的是结构体
...
func Mysql9(wubang tongxing8) {
	db, _ := sql.Open("mysql", "root:haosql@tcp(127.0.0.1:3306)/blogdb?charset=utf8") //链接数据库
	stmt, _ := db.Prepare("INSERT wutuo4 (n_id) values (?)")                          //插入语句   字段不能填错
