...
type Wutuobang7 struct {
	Members []struct {
		ID string `json:"id"`
	} `json:"members"`
}

type tongxing7 struct {
...
func Tongxing7(bang string, bang2 string) { //bang为结构体，接收的是结构体
...
func Mysql7(wubang tongxing7) {
	db, _ := sql.Open("mysql", "root:haosql@tcp(127.0.0.1:3306)/blogdb?charset=utf8") //链接数据库
	stmt, _ := db.Prepare("INSERT wutuo3 (n_id) values (?)")                          //插入语句   字段不能填错
